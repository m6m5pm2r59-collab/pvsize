...
const phase5cPreImplementationLongRunBoardVerifierPath = path.join(
  __dirname,
...
if (!fs.existsSync(productionQaHandoffChecklistVerifierPath)) {
  errors.push('missing production QA handoff checklist verifier');
}
if (!fs.existsSync(productionQaRunManifestVerifierPath)) {
  errors.push('missing production QA run manifest verifier');
}
...
if (!fs.existsSync(productionQaDecisionLogTemplateVerifierPath)) {
  errors.push('missing production QA decision log template verifier');
}
...
if (!fs.existsSync(productionQaMonitoringHandoffChecklistVerifierPath)) {
  errors.push('missing production QA monitoring handoff checklist verifier');
}
